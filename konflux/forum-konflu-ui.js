...
export const fetchCount = (apiUrl, github = false) => {
    ...
    const jiraAuth = JIRA_AUTH

    return fetch(apiUrl, {
        headers: {
            ...(!github ? { 'Authorization': jiraAuth } : {}),
...
export const handler = async(event) => {
    ...
    let count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12396633');

    hac_stories = hac_stories + `\n1. <https://issues.redhat.com/issues/?filter=12396633 | Stories Done> (${count})`

    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12396635')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12396638')

    hac_stories = hac_stories + `\n3. <https://issues.redhat.com/issues/?filter=12396638 | Unassigned stories:> (${count})`


    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12396637')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12396775')

    hac_bugs = hac_bugs + `\n1.  <https://issues.redhat.com/issues/?filter=12396775 | QE Review > (${count})`



    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12416904')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12416902')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12416903')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12396778')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426878')
    
    rhtap_blockers = rhtap_blockers + `\n1. <https://issues.redhat.com/issues/?filter=12426878 | Bloker reds :alert-siren:> (${count})`
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426877')
    
    rhtap_blockers = rhtap_blockers + `\n2. <https://issues.redhat.com/issues/?filter=12426877 | Bloker yellows :large_yellow_circle: > (${count})`
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426876')
    
    rhtap_blockers = rhtap_blockers + `\n3. <https://issues.redhat.com/issues/?filter=12426876 | Bloker greens :green-circle: > (${count})`
    
    let rhtap_critical="\n\n *RHTAP Critical bugs:*";
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426873')
    
    rhtap_critical = rhtap_critical + `\n1. <https://issues.redhat.com/issues/?filter=12426873 | Critical reds :alert-siren:> (${count})`
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426874')
    
    rhtap_critical = rhtap_critical + `\n2. <https://issues.redhat.com/issues/?filter=12426874 | Critical yellows :large_yellow_circle:> (${count})`
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426875')
    
    rhtap_critical = rhtap_critical + `\n3. <https://issues.redhat.com/issues/?filter=12426875 | Critical greens :green-circle:> (${count})`
    
    let rhtap_major="\n\n *RHTAP Major bugs:*";
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426870')
    
    rhtap_major = rhtap_major + `\n1. <https://issues.redhat.com/issues/?filter=12426870 | Major reds :alert-siren:> (${count})`
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426871')
    
    rhtap_major = rhtap_major + `\n2. <https://issues.redhat.com/issues/?filter=12426871 | Major yellows :large_yellow_circle:> (${count})`
    
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12426872')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12405355')

    stonesoup_dashboard = stonesoup_dashboard + `\n1. <https://issues.redhat.com/issues/?filter=12405355 | Blocked> (${count})`

    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12405357')
    ...
    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12405358')

    stonesoup_dashboard = stonesoup_dashboard + `\n3. <https://issues.redhat.com/issues/?filter=12405358 | Stonesoup UI - Needs UX> (${count})`


    count = await fetchCount('https://issues.redhat.com/rest/api/2/search?jql=filter=12405686')
    ...
    count = await fetchCount('https://api.github.com/search/issues?q=repo%3Aopenshift%2Fhac-dev+state%3Aopen&type=issues', true)

    github_data = github_data + `\n<https://github.com/search?q=repo%3Aopenshift%2Fhac-dev+state%3Aopen&type=issues | HAC open PRs>: (${count})`

    count = await fetchCount('https://api.github.com/search/issues?q=repo%3Aopenshift%2Fhac-dev+state%3Aopen+-label%3algtm&type=issues', true)

    github_data = github_data + `\n<https://github.com/search?q=repo%3Aopenshift%2Fhac-dev+state%3Aopen+-label%3algtm&type=issues | HAC PRs without LGTM>: (${count})`

    count = await fetchCount('https://api.github.com/search/issues?q=repo%3Aopenshift%2Fhac-dev+state%3Aopen+-label%3aapproved&type=issues', true)
    ...
    count = await fetchCount('https://api.github.com/search/issues?q=repo%3Aopenshift%2Fhac-dev+repo%3Aopenshift%2Fconsole+repo%3Aopenshift%2Fenhancements+repo%3Aopenshift%2Fconsole-operator+repo%3Aopenshift%2Fapi+-label%3Ado-not-merge%2Fwork-in-progress+-label%3Algtm+author%3Arohitkrai03+author%3Adebsmita1+author%3Ajeff-phillips-18+author%3AinvincibleJai+author%3Anemesis09+author%3Asahil143+author%3Avikram-raj+author%3Achristianvogt+author%3Ajerolimov+author%3AdivyanshiGupta+author%3Arottencandy+author%3Akarthikjeeyar+author%3Aabhinandan13jan+state%3Aopen+created%3A%3C2022-06-19+author%3ALucifergene+author%3Agruselhaus+author%3Alokanandaprabhu&type=repositories', true)
